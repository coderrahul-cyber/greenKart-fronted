...
const SAVED_PHONE_KEY = 'gk_saved_phone';
...
function Checkbox({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <button type="button" onClick={() => onChange(!checked)} className="flex items-center gap-2.5 group">
      <div className="flex items-center justify-center shrink-0 rounded-md transition-all duration-200"
        style={{
          width: 18, height: 18,
...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const res = await login(digits, password, rememberMe);
    setLoading(false);
    if (res.success) {
      if (rememberMe) {
        localStorage.setItem(SAVED_PHONE_KEY, digits);
  ...
  const clearSaved = () => {
    localStorage.removeItem(SAVED_PHONE_KEY);
    localStorage.removeItem(SAVED_PW_KEY);
    setPhone(''); setPassword(''); setRememberMe(false); setHasSaved(false);
  };
      ...
      <div className="rounded-3xl px-7 py-8 sm:px-9 sm:py-10"
        ...
        <div className="mb-8 flex flex-col gap-1.5">
          <span className="text-xs font-semibold tracking-widest uppercase font-roboto"
            style={{ color: "rgba(74,222,128,0.75)" }}>Welcome back</span>
          <h1 className="text-3xl font-semibold text-white font-playfair" style={{ letterSpacing: "-0.02em" }}>Sign in</h1>
              ...
              <div className="flex items-center justify-between gap-3 rounded-2xl px-4 py-3"
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <p className="text-xs font-roboto truncate" style={{ color: 'rgba(255,255,255,0.5)' }}>
                    Saved as <span style={{ color: 'rgba(74,222,128,0.8)' }}>+91 {phone}</span>
                  </p>
                </div>
                <button type="button" onClick={clearSaved}
                  className="shrink-0 text-[10px] font-semibold font-roboto px-2.5 py-1 rounded-lg"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold tracking-widest uppercase text-white/35 font-roboto">
              Phone Number
            </label>
            <div className="relative flex items-center">
              ...
              <div className="absolute left-4 flex items-center gap-1.5 pointer-events-none select-none"
                ...
                <span className="text-sm font-poppins">+91</span>
                <span className="w-px h-4 mx-1" style={{ background: 'rgba(255,255,255,0.15)' }}/>
              </div>
              <input
                type="tel"
                required
                ...
                className="w-full rounded-xl py-3.5 pr-4 text-sm font-poppins text-white placeholder-white/20 outline-none transition-all"
                style={{
                  paddingLeft: '88px',
              ...
              />
              <span className="absolute right-4 text-xs font-roboto pointer-events-none"
          ...
          <motion.button
            ...
            {loading ? (
              <><svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>Signing in…</>
            ) : (
              <>Sign In<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <motion.div
        ...
        <Link href="/">
          <span className="text-3xl font-semibold select-none font-playfair"
            style={{ background: "linear-gradient(135deg, #f0f7ee 30%, #86efac 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            GreenKart
          </span>
        </Link>
      </motion.div>
